...
const AngraphTab: React.FC = () => {
  ...
  const fetchGraphData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`https://cvnhjf6hpj.execute-api.us-east-1.amazonaws.com/getGraphData?minDate=${startDate}&maxDate=${endDate}`, {
        method: "GET",
      ...
      {!loading && !error && (currentSensorData.length > 0) ? (
        <>
          <div className="chart-wrapper">
            <Line data={{ datasets: temperatureDatasets }} options={temperatureChartOptions as any} />
          </div>
          <div className="chart-wrapper" style={{ marginTop: '40px' }}>
            <Line data={{ datasets: humidityDatasets }} options={humidityChartOptions as any} />
          </div>
        </>
      ) : (
        !loading && !error && (
